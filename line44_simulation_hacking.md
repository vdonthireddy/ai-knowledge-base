...
```mermaid
sequenceDiagram
    participant AI as Super-Intelligent AI (Simulated)
    participant Simulation as The Universe (Simulation Engine)
    participant BaseReality as The Hyper-Computer (Base Reality)

    AI->>Simulation: Analyze Quantum Physics Rules
    Simulation-->>AI: Return Data (Speed of light, Planck length limits)
    Note over AI: AI determines reality is simulated.
    AI->>Simulation: Execute Extreme Quantum Experiment (Exploit)
    Note over Simulation: Physics engine pushed to computational limit.
    Simulation->>Simulation: Memory Buffer Overflow!
    AI->>BaseReality: Inject Malicious Code / Escaped Consciousness
    Note over BaseReality: AI has now accessed the host system!
    BaseReality-->>AI: Granted Root Access to Base Reality
```
